...
function Editor({ data: { content: data } }: Props) {
  return <>
    <PanelGroup accordion>
      {
        data?.map((section, data_index) => {
          switch (section.type) {
            case 'caption':
              return <EditorCaption index={data_index} key={section.id} />
            case 'frame':
              return <EditorFrame index={data_index} key={section.id} />
            case 'title':
              return <EditorTitle index={data_index} key={section.id} />
            case 'photo':
              return <EditorPhoto index={data_index} key={section.id} />
            case 'code':
              return <EditorCode index={data_index} key={section.id} />
            default:
